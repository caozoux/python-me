...
int trace_v4_connect(struct pt_regs *ctx, struct sock *sk)
{
    //bpf_trace_printk("new v4 %lx\\n", sk);
    return 0;
};

int trace_v6_connect(struct pt_regs *ctx, struct sock *sk)
{
    //bpf_trace_printk("new v6 %lx\\n", sk);
    return 0;
};
...
int trace_tcp_v4_rcv(struct pt_regs *ctx)
{
    ...
    bpf_probe_read(&source, sizeof(u16), &th->source);
    bpf_probe_read(&dest, sizeof(u16), &th->dest);
    //if (ntohs(dest) == 32032 || ntohs(source) == 32032 )
    if (ntohs(dest) == 50010 || ntohs(source) == 50010 )
            bpf_trace_printk("connect %ld %ld\\n", ntohs(source), ntohs(dest));
    return 0;
};

int trace_tcp_v4_inbound_md5_hash(struct pt_regs *ctx)
{
    struct sock *sk = (struct sock *) ctx->di;
    //if (sk->__sk_common.skc_state == SS_CONNECTING) {
        ...
        bpf_probe_read(&dest, sizeof(u16), &th->dest);
        //if (dest == 32032)
            bpf_trace_printk("connect %lx %ld %ld\\n", sk, ntohs(source), ntohs(dest));
    //}
    return 0;
};
...
int trace_tcp_retransmit_skb(struct pt_regs *ctx)
{
    ...
    struct tcp_skb_cb *tcp_skb = (struct tcp_skb_cb *)&skb->cb[0];

    //if (skp->__sk_common.skc_state == TCP_ESTABLISHED)
    //if (!(tcp_skb->tcp_flags & TCPHDR_SYN))
    if (skp->__sk_common.skc_state != TCP_SYN_SENT)
       return 0;

    bpf_trace_printk("zz %lx %lx\\n", skp, tcp_skb->tcp_flags);
    ...
    if (family == AF_INET) {
        struct ipv4_data_t data4 = {.pid = 0, .ip = 4};
        data4.ts_us = 0;
        ...
        struct ipv6_data_t data6 = {.pid = 0, .ip = 6};
        data6.ts_us = 0;
...
# initialize BPF
b = BPF(text=bpf_text)
#b.attach_kprobe(event="tcp_v4_inbound_md5_hash", fn_name="trace_tcp_v4_inbound_md5_hash")
#b.attach_kprobe(event="tcp_filter", fn_name="trace_tcp_v4_inbound_md5_hash")
#b.attach_kprobe(event="tcp_v6_rcv", fn_name="trace_tcp_v4_rcv")
#b.attach_kprobe(event="tcp_v4_rcv", fn_name="trace_tcp_v4_rcv")
#b.attach_kprobe(event="ip_rcv", fn_name="trace_tcp_v4_rcv")
b.attach_kprobe(event="netif_receive_skb_internal", fn_name="trace_tcp_v4_rcv")
b.attach_kprobe(event="tcp_v6_connect", fn_name="trace_v6_connect")
b.attach_kprobe(event="tcp_v4_connect", fn_name="trace_v4_connect")

b.attach_kprobe(event="__tcp_retransmit_skb",
    fn_name="trace_tcp_retransmit_skb")
...
def print_ipv4_event(cpu, data, size):
    ...
    if args.lport:
        print("%-6d %-12.12s %-2d %-16s %-6d %-16s %-5d %.2f %d" % (event.pid,
            event.task.decode('utf-8', 'replace'), event.ip,
            inet_ntop(AF_INET, pack("I", event.saddr)), event.lport,
            inet_ntop(AF_INET, pack("I", event.daddr)), event.dport,
            float(event.delta_us) / 1000, event.stat))
    else:
        print("%-6d %-12.12s %-2d %-16s %-16s %-5d %.2f %lx" % (event.pid,
            event.task.decode('utf-8', 'replace'), event.ip,
            inet_ntop(AF_INET, pack("I", event.saddr)), event.lport,
            inet_ntop(AF_INET, pack("I", event.daddr)), event.dport,
            float(event.delta_us) / 1000, event.stat))
...
def print_ipv6_event(cpu, data, size):
    ...
    if args.timestamp:
        ...
        print("%-9.3f" % ((float(event.ts_us) - start_ts) / 1000000), end="")
    if args.lport:
        print("%-6d %-12.12s %-2d %-16s %-6d %-16s %-5d %.2f %lx" % (event.pid,
            event.task.decode('utf-8', 'replace'), event.ip,
            inet_ntop(AF_INET6, event.saddr), event.lport,
            inet_ntop(AF_INET6, event.daddr),
            event.dport, float(event.delta_us) / 1000, event.stat))
    else:
