...
int trace_func_entry_stack(struct pt_regs *ctx)
{
...
static void trace_func_entry_handle(u64 addr)
{
    u32 pid;
    u64 ts;
...
static void trace_func_exit_handle(void)
{
    u64 delta;
    u32 pid;
    lock_data *lock;
...
if args.lock or args.type == "per_rwsem_read" or args.type == "per_rwsem_write" or args.type == "per_rwsem_write_run":
    if args.type == "per_rwsem_read" or args.type == "per_rwsem_write" or args.type == "per_rwsem_write_run":
      bpf_text = bpf_text.replace('LOCK', '0')
      bpf_text = bpf_text.replace('LADDR', '0')
      bpf_text = bpf_text.replace('MADDR', '0')
    else:
      bpf_text = bpf_text.replace('LOCK', ('lock.lock != %s')%args.lock)
      bpf_text = bpf_text.replace('LADDR', ('addr != %s')%args.lock)
...
if is_support_kfunc:
  ...
  if args.type == "rwsem_read":
      ...
      bpf_text = bpf_text.replace('HOOK_FUNC', "down_write")
  ...
  elif args.type == "rwsem-stack":
      b.attach_kprobe(event="down_read", fn_name="trace_func_entry_stack")
