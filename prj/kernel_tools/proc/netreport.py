...
def transfer_to_array(line, array_data, split_str=" "):
    """transfter line to array list
    :returns: TODO
...
descirption=["face","bytes","packets","errs","drop","fifo","frame","compressed","multicast","bytes","packets","errs","drop","fifo","colls","carrier","compressed"]

new_totaldata=[]
old_totaldata=[]
for i in range(len(descirption)):
    new_totaldata.append(0)
    old_totaldata.append(0)

for i in range(2, len(lines)):
  line = re.sub(" +"," ",lines[i].strip())
  transfer_to_array(line, new_totaldata)
  print(line)
